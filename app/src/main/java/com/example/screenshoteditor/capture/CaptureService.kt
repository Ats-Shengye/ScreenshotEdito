...
class CaptureService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        android.util.Log.d("CaptureService", "onCreate called")
        try {
            isRunning = true
            android.util.Log.d("CaptureService", "isRunning set to true")
            settingsDataStore = SettingsDataStore(this)
            android.util.Log.d("CaptureService", "SettingsDataStore initialized")
            createNotificationChannel()
            android.util.Log.d("CaptureService", "Notification channel created")
            startForeground(NOTIFICATION_ID, createNotification())
            android.util.Log.d("CaptureService", "Service started in foreground")
        } catch (e: Exception) {
            android.util.Log.e("CaptureService", "Error in onCreate", e)
            isRunning = false
            stopSelf()
        }
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        return try {
                ...
                ACTION_CAPTURE -> {
                    serviceScope.launch {
                        try {
                            ...
                            android.util.Log.e("CaptureService", "Error in handleCaptureAction", e)
                        }
                    }
                }
                ACTION_STOP -> {
            ...
            android.util.Log.e("CaptureService", "Error in onStartCommand", e)
            stopSelf()
            START_NOT_STICKY
        }
    }
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            try {
                ...
                android.util.Log.d("CaptureService", "Notification channel created successfully")
            } catch (e: Exception) {
                android.util.Log.e("CaptureService", "Failed to create notification channel", e)
                throw e
            }
        }
    }
    ...
    private suspend fun handleCaptureAction() {
        ...
        // Start CaptureActivity to handle the actual screenshot
        try {
            ...
            android.util.Log.e("CaptureService", "Error starting CaptureActivity", e)
        }
    }
