...
class MainActivity : AppCompatActivity() {
    ...
    private fun checkNotificationPermission() {
        try {
            ...
            android.util.Log.e("MainActivity", "Error checking notification permission", e)
        }
    }
    
    private fun startCaptureService() {
        try {
            android.util.Log.d("MainActivity", "Starting CaptureService...")
            ...
            // サービス起動後少し待ってからステータス更新
            binding.root.postDelayed({
                updateServiceStatus()
            }, 1000)
            Toast.makeText(this, "撮影サービスを開始しました", Toast.LENGTH_SHORT).show()
        } catch (e: Exception) {
            android.util.Log.e("MainActivity", "Error starting capture service", e)
