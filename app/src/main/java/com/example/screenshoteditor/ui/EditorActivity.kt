...
class EditorActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.d(TAG, "onCreate: Starting EditorActivity")
        binding = ActivityEditorBinding.inflate(layoutInflater)
        setContentView(binding.root)
        
        // ツールバーは現在のレイアウトでは使用しない
        ...
        Log.d(TAG, "onCreate: Image path received: $imagePath")
        
        if (imagePath == null) {
            Log.e(TAG, "onCreate: No image path provided")
    ...
    private fun loadImage() {
        imagePath?.let { path ->
            Log.d(TAG, "loadImage: Loading image from path: $path")
            try {
                val file = File(path)
                Log.d(TAG, "loadImage: File exists: ${file.exists()}, size: ${file.length()} bytes")
                
                originalBitmap = BitmapFactory.decodeFile(path, null)
                if (originalBitmap != null) {
                    Log.d(TAG, "loadImage: Bitmap loaded successfully, size=${originalBitmap!!.width}x${originalBitmap!!.height}")
                    binding.cropView.setBitmap(originalBitmap!!)
                    Log.d(TAG, "loadImage: Bitmap set to CropView")
                } else {
                    Log.e(TAG, "loadImage: Failed to decode bitmap from file")
                    Toast.makeText(this, R.string.message_screenshot_failed, Toast.LENGTH_SHORT).show()
                    finish()
                }
            } catch (e: Exception) {
                Log.e(TAG, "loadImage: Exception occurred", e)
                e.printStackTrace()
                Toast.makeText(this, R.string.message_screenshot_failed, Toast.LENGTH_SHORT).show()
                finish()
            }
